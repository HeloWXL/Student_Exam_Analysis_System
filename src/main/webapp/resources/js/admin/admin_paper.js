    ...
    switch (obj.event) {
      case 'add':
        layer.open({
          ...
          '                <div class="layui-input-block">' +
          '                    <input type="text" name="scoreSum" id="scoreSum" class="layui-input" disabled="disabled">\n' +
          '                </div>' +
          '            </div>' +
          '        </form>\n' +
          '    </div>\n' +
          ...
          btn1: function(index) {
           ...
           if(paperName ==null&&paperName==''){
             layer.msg("试卷名称不能为空",{icon:5,time:1500});
             return;
           }
           var msg = /\d/;
            //选择题数量
            var selectNum = parseInt($.trim($('#selectNum').val()));

            if(!selectNum.test(msg)&&selectNum!==''){
              layer.msg("选择题分数数据格式不正确",{icon:1,time:1500});
              return;
            }
            //选择题分数
            var selectScore = parseInt($.trim($('#selectScore').val()));
            //填空题数量
            var completionNum = parseInt($.trim($('#completionNum').val()));
            //填空题分数
            var completionScore = parseInt($.trim($('#completionScore').val()));
      ...
      case 'update':
        if (data.length == 0) {
          ...
          layer.open({
            ...
            '                <label class="layui-form-label" style="padding-left:-50px;">选择题数量:</label>\n' +
            '                <div class="layui-input-block">\n' +
            '                    <input type="text"  lay-verify="questionnaireName" name="selectNum" id="selectNum" class="layui-input" disabled="disabled">\n' +
            '                </div>\n' +
            '            </div>\n' +
            '            <div class="layui-form-item">\n' +
            '                <label class="layui-form-label" style="padding-left:-50px;">填空题数量:</label>\n' +
            '                <div class="layui-input-block">\n' +
            '                    <input type="text"  lay-verify="questionnaireName" placeholder="请输入HTML片段" name="completionNum" id="completionNum" class="layui-input" disabled="disabled">\n' +
            '                </div>\n' +
            '            </div>\n' +
            '        </form>\n' +
            '    </div>\n' +
            ...
            btn1: function(index) {
              ...
              var paperName = $.trim($('#paperName').val());
              if (paperName == null && paperName == '') {
                layer.msg('试卷名不能为空', {icon: 5, time: 1500});
                return;
              }
              var testId = $.trim($('select[name="testId"]').val());
              if (testId == null && testId == '') {
                layer.msg('考试名称不能为空', {icon: 5, time: 1500});
                return;
              }
              // 提交
              var paper = {
